...
When working in busy AWS accounts that host multiple apps owned by different teams with lots of infrastructure and complex, dynamic permission policies generated via IaC (e.g. Terraform), it can be hard to guarantee everything will behave as you expect, particularly in higher environments. Sure you can run tests in non-prod, but these can take a long time to highlight an issue, edge cases can be missed or require you to code up convoluted requests to exercise lesser used permissions, and policies or access conditions may differ across environments. Furthermore, you generally don't want to make a tangible impact in your production environment for the sake of testing, such as writing objects to an S3 bucket, and you certainly don't want to give permissions to roles just to run tests.
...
The outcome is the ability to create a suite of repeatable tests for your policies that can alert you to inadvertent permission changes, highlight areas of least privilege, cover pesky edge cases and verify access to your resources from other accounts, all without touching your deployed resources other than obtaining their policies.

## Why

In the majority of cases where I've used the policy simulator console UI, I've been troubleshooting a role's access (or denial of access) to S3 objects at specific paths, which requires evaluating the result using both the role's policies and the bucket policy. Adding a set of context values, test actions and S3 object ARNs (Amazon Resource Names, which specify a resource unambiguously across all of AWS) is fine for a one off, but it's not something you want to repeat often and isn't feasible for ongoing verification.
...
In many cases, role and bucket policies need to be evaluated together in order to allow reading certain objects but not others, so testing one without the other isn't overly useful.

There are other solutions around providing a friendly wrapper for the policy simulator API, but I don't believe any provide the ability to test a role with a resource policy.

## So...

We can get around this by lying to the API.

There is another simulation method, `simulate custom policy`. This, too, won't work with resource policies if using a role entity, but as we provide all the policies for the simulation rather than the simulator fetching them from actual entities itself, we can tell it we're testing a user ARN while supplying our role's policy and it won't notice.

One caveat here is that as we're not able to test roles directly, then in order to work straight off the bat this requires resource policies to use non-specific principals like `AWS: *` or your account root ARN, then target specific ARNs with `Conditions` referencing, for example, the `aws:PrincipalArn` context value which we then set in our requests. This is so our dummy user ARN will match against the principals that the statement in general will apply to, but the resulting allow or deny from the `Conditions` will be evaluated as if our role is trying to access the resource. If you use explicit principals, it should be possible to modify (fetched copies of!) policies to include the dummy user as a principal prior to including in the API request, but I haven't tried.
...
This approach is an effective way to keep tabs on evolving permission sets and to provide ongoing verification that bucket policies do what they need to do. It's particularly handy for roles you don't own or can't use directly and therefore won't feature in your day-to-day testing, as it gives you confidence that other users will be able to successfully read the data they need (subject to correct permissions on their end), while being denied access to other areas. You do need to know and understand the actions required by your roles for defining the tests, but even the initial configuration process can indicate areas where unnecessary permissions are granted.
